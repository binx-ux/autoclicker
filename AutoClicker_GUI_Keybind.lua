...
local abilityKey        = Enum.KeyCode.Q      -- locked Slash of Fury key (unused / disabled)

local manualSpamActive  = false
local triggerbotOn      = false
local slashOfFuryOn     = false -- disabled
    ...
    bugButton.MouseButton1Click:Connect(function()
        ...
        task.spawn(function()
            ...
            if ok then
                ...
                bugStatusLabel.TextColor3 = Color3.fromRGB(255,120,120)
                bugStatusLabel.Text = "Failed ("..tostring(err or "unknown")..")"
            end
        end)
    end)
end
...
---------------------------------------------------------------------//
-- OTHERS PAGE (SCROLLING)
---------------------------------------------------------------------//
local function makeOthersCard(parent)
...
do
    ...
    othersScroll.Size = UDim2.new(1,-20,1,-100)
    othersScroll.Position = UDim2.new(0,10,0,90)
    ...
    -- Discord card
    -----------------------------------------------------------------//
    local discordCard = makeOthersCard(othersScroll)
    local dTitle = makeTitle(discordCard,"Discord")
    local dDesc  = makeDesc(discordCard,"Join:  discord.gg/S4nPV2Rx7F")
    ...
    -- FPS Booster
    -----------------------------------------------------------------//
    local fpsCard = makeOthersCard(othersScroll)
    local fTitle = makeTitle(fpsCard,"FPS Booster")
    local fDesc  = makeDesc(fpsCard,"Turn graphics low + remove shadows to help FPS (client-side).")
    ...
    -- Player Effects
    -----------------------------------------------------------------//
    local peCard = makeOthersCard(othersScroll)
    local peTitle = makeTitle(peCard,"Player Effects")
    local peDesc  = makeDesc(peCard,"Activates korblox and headless effects (local visual).")
    ...
    -----------------------------------------------------------------//
    local espCard = makeOthersCard(othersScroll)
    local espTitle = makeTitle(espCard,"Ability ESP (Unavailable)")
    local espDesc  = makeDesc(espCard,"Temporarily unavailable.")
...
local function makeBlatantTitle(parent,txt)
    local t = Instance.new("TextLabel")
...
local function makeBlatantDesc(parent,txt)
    local d = Instance.new("TextLabel")
...
---------------------------------------------------------------------//
local semiCard = createBlatantCard(52)
makeBlatantTitle(semiCard,"Semi Immortal")
makeBlatantDesc(semiCard,"Temporarily unavailable.")
...
---------------------------------------------------------------------//
local setCard = createBlatantCard(52)
makeBlatantTitle(setCard,"Settings")
makeBlatantDesc(setCard,"Semi-Immortal configuration (currently disabled).")
...
local slashCard = createBlatantCard(52)
makeBlatantTitle(slashCard,"Slash of Fury Detection")
makeBlatantDesc(slashCard,"Temporarily unavailable.")
...
local speedCard = createBlatantCard(64)
makeBlatantTitle(speedCard,"Speed")
makeBlatantDesc(speedCard,"Choose the speed of your character. Toggle must be ON to apply.")
...
speedToggleFrame.Size = UDim2.new(0,40,0,20)
speedToggleFrame.Position = UDim2.new(1,-50,0,8)
speedToggleFrame.BackgroundColor3 = Color3.fromRGB(40,40,48)
speedToggleFrame.BorderSizePixel = 0
speedToggleFrame.ZIndex = 4
speedToggleFrame.Parent = speedCard
...
speedThumb.ZIndex = 5
speedThumb.Parent = speedToggleFrame
...
local function updateSpeedToggleVisual()
    if speedEnabled then
        speedToggleFrame.BackgroundColor3 = Color3.fromRGB(80,200,120)
...
local function createSlider(title,desc,min,max,default,callback)
    local card = createBlatantCard(64)
    makeBlatantTitle(card,title)
    makeBlatantDesc(card,desc)
...
---------------------------------------------------------------------//
local manualCard = createBlatantCard(48)
makeBlatantTitle(manualCard,"Manual Spam")
makeBlatantDesc(manualCard,"Spam push on keypress.")
...
local triggerCard = createBlatantCard(48)
makeBlatantTitle(triggerCard,"Triggerbot")
makeBlatantDesc(triggerCard,"Block instant when they target you. (Here: constant parry spam.)")
...
UIS.InputBegan:Connect(function(input,gp)
    ...
    -- Slash of Fury disabled, so abilityKey/Q does nothing
